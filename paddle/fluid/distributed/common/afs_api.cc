...
void Zerr(int ret) {
    switch (ret) {
        case Z_ERRNO:
            if (ferror(stdin))
                LOG(ERROR) << "zpipe: error reading stdin";
            if (ferror(stdout))
                LOG(ERROR) << "zpipe: error writing stdout";
            break;
        case Z_STREAM_ERROR:
...
AfsWriter::AfsWriter(afs::AfsFileSystem* afshandler, std::string filename) {
    ...
    if (_writer == NULL) {
        LOG(ERROR) << "fail to open writer for file " << filename << " errno: " << afs::GetRc() << " errmsg: " << afs::Rc2Str(afs::GetRc());
        exit(-1);
    }
    ...
    if (deflateInit2(&_strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, MAX_WBITS + 16, 8, Z_DEFAULT_STRATEGY) != Z_OK) {
...
int AfsWriter::write(const uint64_t k, const std::vector<float>& value) {
    uint32_t len = sizeof(uint64_t) + value.size() * sizeof(float) + sizeof(uint32_t);
    if (len + _read_count >= CHUNK) {
        _strm.next_in = (Bytef*)_line_buf;
        _strm.avail_in = _read_count;
        compress(Z_NO_FLUSH);
...
int AfsWriter::compress(int mode) {
    ...
    if (!_use_gz) {
        ...
        if (res_len < 0) {
            LOG(WARNING) << "fail to append, expect_len: " << _strm.avail_in << ", errno: " << res_len << ", errmsg:" << afs::Rc2Str(res_len);
            exit(-1);
        } else if (static_cast<uint32_t>(res_len) != _strm.avail_in) {
            LOG(WARNING) << "fail to append, expect_len: " << _strm.avail_in << ", res_len: " << res_len;
            exit(-1);
        }
        return 0;
    }
    ...
    do {
        ...
        int have = CHUNK - _strm.avail_out;
        {
            ...
            if (res_len < 0) {
                LOG(WARNING) << "fail to append, expect_len: " << have << ", errno: " << res_len << ", errmsg:" << afs::Rc2Str(res_len);
                exit(-1);
            } else if (res_len != have) {
                LOG(WARNING) << "fail to append, expect_len: " << have << ", res_len: " << res_len;
                exit(-1);
            }
        }
...
int AfsApiWrapper::init(const char* fs_name, const char* fs_user, const char* pass, const char* conf) {
    _afshandler = new afs::AfsFileSystem(fs_name, fs_user, pass, "./conf/client.conf");
...
int AfsApiWrapper::upload_file(const std::string& local_file, const std::string& afs_file) {
...
int AfsApiWrapper::download_file(const std::string& local_file, const std::string& afs_file) {
...
int AfsApiWrapper::mv(const std::string& old_path, const std::string& dest_path) {
...
std::shared_ptr<AfsWriter> AfsApiWrapper::open_writer(const std::string& filename) {
    return std::make_shared<AfsWriter>(_afshandler, filename);
}

std::shared_ptr<AfsReader> AfsApiWrapper::open_reader(const std::string& filename) {
